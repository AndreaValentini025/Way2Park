...
parks = get_nearby_parks(parks, searchedLat, searchedLong)


#creo un array di dizionari con tutte le info
park_list = []
for park in parks:
    walking_distance, walking_time = calculate_walking_distance(park['lat'], park['long'], searchedLat, searchedLong)
    driving_distance, driving_time = calculate_driving_distance(positionLat, positionLong, park['lat'], park['long'])
    park_info = {
        'id': park['id'],
